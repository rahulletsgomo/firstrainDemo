...
(function (a, b) {
    ...
    function bZ(a, b, c, d) {
        ...
        }); else if (!c && b != null && typeof b == "object")for (var e in b)bZ(a + "[" + e + "]", b[e], c, d); else d(a, b)
    }
    ...
    f.extend({_Deferred:function () {
        ...
        if (a.attachEvent)for (p in{submit:1, change:1, focusin:1})o = "on" + p, q = o in a, q || (a.setAttribute(o, "return;"), q = typeof a[o] == "function"), i[p + "Bubbles"] = q;
        return i
    ...
    be.optgroup = be.option, be.tbody = be.tfoot = be.colgroup = be.caption = be.thead, be.th = be.td, f.support.htmlSerialize || (be._default = [1, "div<div>", "</div>"]), f.fn.extend({text:function (a) {
        ...
        for (var i = 0, j; (j = a[i]) != null; i++) {
            typeof j == "number" && (j += "");
            if (!j)continue;
            if (typeof j == "string")if (!ba.test(j))j = b.createTextNode(j); else {
                j = j.replace(Z, "<$1></$2>");
            ...
            var r;
            if (!f.support.appendChecked)if (j[0] && typeof (r = j.length) == "number")for (i = 0; i < r; i++)bk(j[i]); else bk(j);
        ...
        if (d) {
            ...
            for (i = 0; h[i]; i++)if (e && f.nodeName(h[i], "script") && (!h[i].type || h[i].type.toLowerCase() === "text/javascript"))e.push(h[i].parentNode ? h[i].parentNode.removeChild(h[i]) : h[i]); else {
                if (h[i].nodeType === 1) {
    ...
    var ch = {}, ci, cj, ck = /^(?:toggle|show|hide)$/, cl = /^([+\-]=)?([\d+.\-]+)([a-z%]*)$/i, cm, cn = [
